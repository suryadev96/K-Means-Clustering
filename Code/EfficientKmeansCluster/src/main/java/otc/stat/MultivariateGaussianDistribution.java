...
import otc.math.matrix.Cholesky;
import otc.math.matrix.DenseMatrix;
import otc.math.matrix.Matrix;

public class MultivariateGaussianDistribution extends AbstractMultivariateDistribution
	...
	/**
	 * Constructor. The distribution will have a diagonal covariance matrix of the
	 * same variance.
	 *
	 * @param mean
	 *            mean vector.
	...
	/**
	 * Constructor. The distribution will have a diagonal covariance matrix. Each
	 * element has different variance.
	 *
	 * @param mean
	 *            mean vector.
	 * @param var
	...
	public MultivariateGaussianDistribution(double[][] data, boolean diagonal) {
		...
		if (diagonal) {
			...
			sigma = Matrix.newInstance(Maths.cov(data, mu));
		}
	...
	/**
	 * Algorithm from Alan Genz (1992) Numerical Computation of Multivariate Normal
	 * Probabilities, Journal of Computational and Graphical Statistics, pp.
	 * 141-149.
	 *
	 * The difference between returned value and the true value of the CDF is less
	 * than 0.001 in 99.9% time. The maximum number of iterations is set to 10000.
	 */
	@Override
