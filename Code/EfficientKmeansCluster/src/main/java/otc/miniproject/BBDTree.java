...
public class BBDTree {
	 ...
	 *
	 * If c denotes the mean of mass of the data in this node and n denotes the
	 * number of data in it, then this quantity is given by
	 *
	 * n * ||c - mean||^2 + sum_{x \in node} ||x - c||^2
	 *
	 * The sum is precomputed for each node as cost. This formula follows from
	 * expanding both sides as dot products.
	 */
	...
	/**
	 * Given k cluster centroids, this method assigns data to nearest centroids. The
	 * return value is the distortion to the centroids. The parameter sums will hold
	 * the sum of data for each cluster. The parameter counts hold the number of
	 * data of each cluster. If membership is not null, it should be an array of
	 * size n that will be filled with the index of the cluster [0 - k) that each
	...
	/**
	 * This determines which clusters all data that are rooted node will be assigned
	 * to, and updates sums, counts and membership (if not null) accordingly.
	 * Candidates maintains the set of cluster indices which could possibly be the
	 * closest clusters for data in this subtree.
	 */
	private double filter(Node node, double[][] centroids, int[] candidates, int k, double[][] sums, int[] counts,
			int[] membership) {
		int d = centroids[0].length;

		// Determine which mean the node mean is closest to
		double minDist = Mathstemp.squaredDistance(node.center, centroids[candidates[0]]);
		int closest = candidates[0];
		for (int i = 1; i < k; i++) {
			double dist = Mathstemp.squaredDistance(node.center, centroids[candidates[i]]);
			if (dist < minDist) {
				minDist = dist;
		...
		if (node.lower != null) {
			...
			if (newk > 1) {
				double result = filter(node.lower, centroids, newCandidates, newk, sums, counts, membership)
						+ filter(node.upper, centroids, newCandidates, newk, sums, counts, membership);
	...
	/**
	 * Determines whether every point in the box is closer to centroids[bestIndex]
	 * than to centroids[testIndex].
	 *
	 * If x is a point, c_0 = centroids[bestIndex], c = centroids[testIndex], then:
	 * (x-c).(x-c) < (x-c_0).(x-c_0) <=> (c-c_0).(c-c_0) < 2(x-c_0).(c-c_0)
	 *
	 * The right-hand side is maximized for a vertex of the box where for each
	 * dimension, we choose the low or high value based on the sign of x-c_0 in that
	 * dimension.
	 */
