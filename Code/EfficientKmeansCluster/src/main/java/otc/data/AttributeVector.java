...
public class AttributeVector {
	 ...
	 *            the data vector.
	 * @param names
	 *            optional names for each element.
	 */
	...
	public AttributeVector summary() {
		...
		stat[0] = Maths.min(vector);
		stat[1] = Maths.q1(vector);
		stat[2] = Maths.median(vector);
		stat[3] = Maths.mean(vector);
		stat[4] = Maths.q3(vector);
		stat[5] = Maths.max(vector);
