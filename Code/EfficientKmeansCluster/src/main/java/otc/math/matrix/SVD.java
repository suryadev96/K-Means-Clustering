...
public class SVD {
	...
	public SVD(DenseMatrix U, DenseMatrix V, double[] s) {
		...
		tol = 0.5 * Maths.sqrt(m + n + 1.0) * s[0] * Maths.EPSILON;
	}
	...
	/**
	 * Returns the one-dimensional array of singular values, ordered by from largest
	 * to smallest.
	 */
	...
	/**
	 * Returns the dimension of null space. The number of negligible singular
	 * values.
	 */
	public int nullity() {
		if (!full) {
	...
	/**
	 * Returns the L<sub>2</sub> norm condition number, which is max(S) / min(S). A
	 * system of equations is considered to be well-conditioned if a small change in
	 * the coefficient matrix or a small change in the right hand side results in a
	 * small change in the solution vector. Otherwise, it is called ill-conditioned.
	 * Condition number is defined as the product of the norm of A and the norm of
	 * A<sup>-1</sup>. If we use the usual L<sub>2</sub> norm on vectors and the
	 * associated matrix norm, then the condition number is the ratio of the largest
	 * singular value of matrix A to the smallest. Condition number depends on the
	 * underlying norm. However, regardless of the norm, it is always greater or
	 * equal to 1. If it is close to one, the matrix is well conditioned. If the
	 * condition number is large, then the matrix is said to be ill-conditioned. A
	 * matrix that is not invertible has the condition number equal to infinity.
	 */
	...
	/**
	 * Returns a matrix of which columns give an orthonormal basis for the range
	 * space.
	 */
	...
	/**
	 * Returns a matrix of which columns give an orthonormal basis for the null
	 * space.
	 */
	 ...
	 * @param x
	 *            the output solution vector that minimizes the L2 norm of Q*R*x -
	 *            b.
	 ...
	 * 
	 * @param B
	 *            right hand side of linear system. B will be overwritten with the
	 *            solution matrix on output.
	 * @exception RuntimeException
