...
public class Lanczos {
	...
	public static EVD eigen(Matrix A, int k, double kappa, int maxIter) {
		...
		double eps = Maths.EPSILON * Maths.sqrt(n);
		double reps = Maths.sqrt(Maths.EPSILON);
		double eps34 = reps * Maths.sqrt(reps);
		kappa = Maths.max(kappa, eps34);
		...
		double t = 1.0 / rnm;
		Maths.scale(t, wptr[0], wptr[1]);
		Maths.scale(t, wptr[3]);

		// take the first step
		A.ax(wptr[3], wptr[0]);
		alf[0] = Maths.dot(wptr[0], wptr[3]);
		Maths.axpy(-alf[0], wptr[1], wptr[0]);
		t = Maths.dot(wptr[0], wptr[3]);
		Maths.axpy(-t, wptr[1], wptr[0]);
		alf[0] += t;
		Maths.copy(wptr[0], wptr[4]);
		rnm = Maths.norm(wptr[0]);
		double anorm = rnm + Maths.abs(alf[0]);
		...
		int last = Maths.min(k + Maths.max(8, k), n);
		...
		for (; !enough && iter < maxIter; iter++) {
			...
			for (j = first; j < last; j++) {
				...
				Maths.scale(t, wptr[3]);
				A.ax(wptr[3], wptr[0]);
				Maths.axpy(-rnm, wptr[2], wptr[0]);
				alf[j] = Maths.dot(wptr[0], wptr[3]);
				Maths.axpy(-alf[j], wptr[1], wptr[0]);
				...
				for (int i = 0; i < Math.min(ll, j - 1); i++) {
					t = Maths.dot(p[i], wptr[0]);
					Maths.axpy(-t, q[i], wptr[0]);
					eta[i] = eps;
					oldeta[i] = eps;
				}

				// extended local reorthogonalization
				t = Maths.dot(wptr[0], wptr[4]);
				Maths.axpy(-t, wptr[2], wptr[0]);
				if (bet[j] > 0.0) {
					bet[j] = bet[j] + t;
				}
				t = Maths.dot(wptr[0], wptr[3]);
	...
	/**
	 * Generate a starting vector in r and returns |r|. It returns zero if the range
	 * is spanned, and throws exception if no starting vector within range of
	...
	private static double startv(Matrix A, double[][] q, double[][] wptr, int step) {
		// get initial vector; default is random
		double rnm = Maths.dot(wptr[0], wptr[0]);
		double[] r = wptr[0];
		...
		if (step > 0) {
			for (int i = 0; i < step; i++) {
				double t = Maths.dot(wptr[3], q[i]);
				Maths.axpy(-t, q[i], wptr[0]);
			}

			// make sure q[step] is orthogonal to q[step-1]
			double t = Maths.dot(wptr[4], wptr[0]);
			Maths.axpy(-t, wptr[2], wptr[0]);
			Maths.copy(wptr[0], wptr[3]);
			t = Maths.dot(wptr[3], wptr[0]);
			if (t <= Maths.EPSILON * rnm) {
				t = 0.0;
			}
			rnm = t;
		}
	 ...
	 * 
	 * @param alf
	 *            array to store diagonal of the tridiagonal matrix T
	 * @param bet
	 *            array to store off-diagonal of T
	 * @param eta
	 *            on input, orthogonality estimate of Lanczos vectors at step j. On
	 *            output, orthogonality estimate of Lanczos vectors at step j+1 .
	 * @param oldeta
	 *            on input, orthogonality estimate of Lanczos vectors at step j-1 On
	 *            output orthogonality estimate of Lanczos vectors at step j
	 * @param step
	 *            dimension of T
	 * @param rnm
	 *            norm of the next residual vector
	 * @param eps
	 *            roundoff estimate for dot product of two unit vectors
	 */
	private static void ortbnd(double[] alf, double[] bet, double[] eta, double[] oldeta, int step, double rnm,
			double eps) {
		if (step < 1) {
			return;
		}
		...
		if (0 != rnm) {
			...
			for (int i = 1; i <= step - 2; i++) {
				oldeta[i] = (bet[i + 1] * eta[i + 1] + (alf[i] - alf[step]) * eta[i] + bet[i] * eta[i - 1]
						- bet[step] * oldeta[i]) / rnm + eps;
			}
		}
	...
	/**
	 * Examine the state of orthogonality between the new Lanczos vector and the
	 * previous ones to decide whether re-orthogonalization should be performed.
	 * 
	 * @param ll
	 *            number of intitial Lanczos vectors in local orthog.
	 * @param r
	 *            on input, residual vector to become next Lanczos vector. On
	 *            output, residual vector orthogonalized against previous Lanczos.
	 * @param q
	 *            on input, current Lanczos vector. On Output, current Lanczos
	 *            vector orthogonalized against previous ones.
	 * @param ra
	 *            previous Lanczos vector
	 * @param qa
	 *            previous Lanczos vector
	 * @param eta
	 *            state of orthogonality between r and prev. Lanczos vectors
	 * @param oldeta
	 *            state of orthogonality between q and prev. Lanczos vectors
	 */
	...
	private static int idamax(int n, double[] dx, int ix0, int incx) {
		...
		imax = ix;
		dmax = Maths.abs(dx[ix]);
		for (int i = 1; i < n; i++) {
			ix += incx;
			double dtemp = Maths.abs(dx[ix]);
			if (dtemp > dmax) {
				dmax = dtemp;
				imax = ix;
			}
	 ...
	 * 
	 * @param ritz
	 *            array to store the ritz values
	 * @param bnd
	...
	private static int error_bound(boolean[] enough, double[] ritz, double[] bnd, int step, double tol, double eps34) {
		...
		for (int i = ((step + 1) + (step - 1)) / 2; i >= mid + 1; i -= 1) {
			if (Maths.abs(ritz[i - 1] - ritz[i]) < eps34 * Maths.abs(ritz[i])) {
				if (bnd[i] > tol && bnd[i - 1] > tol) {
					bnd[i - 1] = Maths.sqrt(bnd[i] * bnd[i] + bnd[i - 1] * bnd[i - 1]);
					bnd[i] = 0.0;
				}
			}
		}
		...
		for (int i = 0; i <= step; i++) {
			...
			if (bnd[i] <= 16.0 * Maths.EPSILON * Maths.abs(ritz[i])) {
				neig++;
				if (!enough[0]) {
					enough[0] = -Maths.EPSILON < ritz[i] && ritz[i] < Maths.EPSILON;
				}
			}
		}

		logger.info("Lancozs method found {} converged eigenvalues of the {}-by-{} matrix", neig, step + 1);
		if (neig != 0) {
			for (int i = 0; i <= step; i++) {
				if (bnd[i] <= 16.0 * Maths.EPSILON * Maths.abs(ritz[i])) {
	...
	/**
	 * Based on the input operation flag, stores to or retrieves from memory a
	 * vector.
	 * 
	 * @param s
	 *            contains the vector to be stored
	 */
