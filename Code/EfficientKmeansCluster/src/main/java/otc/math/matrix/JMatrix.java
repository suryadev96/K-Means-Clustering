...
public class JMatrix extends DenseMatrix {
	...
	/**
	 * Constructor of a column vector/matrix with given array as the internal
	 * storage.
	 * 
	 * @param A
	...
	public JMatrix abmm(DenseMatrix B) {
		if (ncols() != B.nrows()) {
			throw new IllegalArgumentException(String.format("Matrix multiplication A * B: %d x %d vs %d x %d", nrows(),
					ncols(), B.nrows(), B.ncols()));
		}
	...
	/**
	 * LU decomposition is computed by a "left-looking", dot-product,
	...
	/**
	 * Cholesky decomposition for symmetric and positive definite matrix. Only the
	 * lower triangular part will be used in the decomposition.
	 *
	...
	public SVD svd() {
		...
		for (k = n - 1; k >= 0; k--) {
			for (its = 0; its < 30; its++) {
				...
				for (l = k; l >= 0; l--) {
					nm = l - 1;
					if (l == 0 || Maths.abs(rv1[l]) <= Maths.EPSILON * anorm) {
						flag = false;
						break;
					}
					if (Maths.abs(w[nm]) <= Maths.EPSILON * anorm) {
						break;
					}
				}
				...
				if (flag) {
					...
					for (i = l; i < k + 1; i++) {
						f = s * rv1[i];
						rv1[i] = c * rv1[i];
						if (Maths.abs(f) <= Maths.EPSILON * anorm) {
							break;
						}
						g = w[i];
						h = Math.hypot(f, g);
	 ...
	 *
	 * The routine works extremely well in practice. The number of iterations for
	 * the first few eigenvalues might be 4 or 5, say, but meanwhile the
	 * off-diagonal elements in the lower right-hand corner have been reduced too.
	 * The later eigenvalues are liberated with very little work. The average number
	 * of iterations per eigenvalue is typically 1.3 - 1.6. The operation count per
	 * iteration is O(n), with a fairly large effective coefficient, say, ~20n. The
	 * total operation count for the diagonalization is then ~20n * (1.3 - 1.6)n =
	 * ~30n^2. If the eigenvectors are required, there is an additional, much
	 * larger, workload of about 3n^3 operations.
	 *
	 * @param d
	 *            on input, it contains the diagonal elements of the tridiagonal
	 *            matrix. On output, it contains the eigenvalues.
	 * @param e
	 *            on input, it contains the subdiagonal elements of the tridiagonal
	 *            matrix, with e[0] arbitrary. On output, its contents are
	 *            destroyed.
	 * @param n
	 *            the size of all parameter arrays.
	 */
	private static void tql(double[] d, double[] e, int n) {
		...
		for (int l = 0; l < n; l++) {
			...
			int m = l;
			for (; m < n; m++) {
				if (Maths.abs(e[m]) <= Maths.EPSILON * tst1) {
					break;
				}
			}
			...
			if (m > l) {
				...
				} while (Maths.abs(e[l]) > Maths.EPSILON * tst1);
			}
			d[l] = d[l] + f;
			e[l] = 0.0;
		}
	...
	/**
	 * Tridiagonal QL Implicit routine for computing eigenvalues and eigenvectors of
	 * a symmetric, real, tridiagonal matrix.
	 *
	 * The routine works extremely well in practice. The number of iterations for
	 * the first few eigenvalues might be 4 or 5, say, but meanwhile the
	 * off-diagonal elements in the lower right-hand corner have been reduced too.
	 ...
	 *
	 * @param V
	 *            on input, it contains the identity matrix. On output, the kth
	 *            column of V returns the normalized eigenvector corresponding to
	 *            d[k].
	 * @param d
	 *            on input, it contains the diagonal elements of the tridiagonal
	 *            matrix. On output, it contains the eigenvalues.
	...
	private static double[] balance(DenseMatrix A) {
		double sqrdx = Maths.RADIX * Maths.RADIX;
	 ...
	 * reduction of a real nonsymmetric matrix to upper Hessenberg form by elmhes.
	 */
	...
	private static void hqr(DenseMatrix A, double[] d, double[] e) {
		...
		while (nn >= 0) {
			...
			do {
				for (l = nn; l > 0; l--) {
					s = Maths.abs(A.get(l - 1, l - 1) + Maths.abs(A.get(l, l)));
					if (s == 0.0) {
						s = anorm;
					}
					if (Maths.abs(A.get(l, l - 1)) <= Maths.EPSILON * s) {
						A.set(l, l - 1, 0.0);
						break;
					}
				}
				...
				if (l == nn) {
					...
					if (l == nn - 1) {
						...
						for (m = nn - 2; m >= l; m--) {
							...
							u = Maths.abs(A.get(m, m - 1)) * (Maths.abs(q) + Maths.abs(r));
							v = Maths.abs(p)
									* (Maths.abs(A.get(m - 1, m - 1)) + Maths.abs(z) + Maths.abs(A.get(m + 1, m + 1)));
							if (u <= Maths.EPSILON * v) {
								break;
							}
						}
						for (i = m; i < nn - 1; i++) {
						...
						for (k = m; k < nn; k++) {
							if (k != m) {
								...
								if ((x = Maths.abs(p) + Maths.abs(q) + Maths.abs(r)) != 0.0) {
									p /= x;
									q /= x;
									r /= x;
								}
							}
							if ((s = Maths.copySign(Maths.sqrt(p * p + q * q + r * r), p)) != 0.0) {
								if (k == m) {
									if (l != m) {
	...
	/**
	 * Finds all eigenvalues of an upper Hessenberg matrix A[0..n-1][0..n-1]. On
	 * input A can be exactly as output from elmhes and eltran. On output, d and e
	 * contain the eigenvalues of A, while V is a matrix whose columns contain the
	 * corresponding eigenvectors. The eigenvalues are not sorted, except that
	 * complex conjugate pairs appear consecutively with the eigenvalue having the
	 * positive imaginary part. For a complex eigenvalue, only the eigenvector
	 * corresponding to the eigenvalue with positive imaginary part is stored, with
	 * real part in V[0..n-1][i] and imaginary part in V[0..n-1][i+1]. The
			...
			do {
				...
				if (l == nn) {
					...
					if (l == nn - 1) {
						p = 0.5 * (y - x);
						q = p * p + w;
						z = Maths.sqrt(Math.abs(q));
						x += t;
						A.set(nn, nn, x);
						A.set(nn - 1, nn - 1, y + t);
						if (q >= 0.0) {
							z = p + Maths.copySign(z, p);
							d[nn - 1] = d[nn] = x + z;
							if (z != 0.0) {
								d[nn] = x - w / z;
							}
		...
		if (anorm != 0.0) {
			for (nn = n - 1; nn >= 0; nn--) {
				...
				if (q == 0.0) {
					...
					for (i = nn - 1; i >= 0; i--) {
						...
						if (e[i] < 0.0) {
							...
							if (e[i] == 0.0) {
								t = w;
								if (t == 0.0) {
									t = Maths.EPSILON * anorm;
								}
								A.set(i, nn, -r / t);
							} else {
								x = A.get(i, i + 1);
								y = A.get(i + 1, i);
								q = Maths.sqr(d[i] - p) + Maths.sqr(e[i]);
								t = (x * s - z * r) / q;
								A.set(i, nn, t);
							...
							t = Maths.abs(A.get(i, nn));
							if (Maths.EPSILON * t * t > 1) {
								for (j = i; j <= nn; j++) {
									A.div(j, nn, t);
								}
							}
					...
					m = na;
					if (Maths.abs(A.get(nn, na)) > Maths.abs(A.get(na, nn))) {
					...
					for (i = nn - 2; i >= 0; i--) {
						...
						if (e[i] < 0.0) {
							...
							if (e[i] == 0.0) {
								...
								x = A.get(i, i + 1);
								y = A.get(i + 1, i);
								vr = Maths.sqr(d[i] - p) + Maths.sqr(e[i]) - q * q;
								vi = 2.0 * q * (d[i] - p);
								if (vr == 0.0 && vi == 0.0) {
									vr = Maths.EPSILON * anorm
											* (Maths.abs(w) + Maths.abs(q) + Maths.abs(x) + Maths.abs(y) + Maths.abs(z));
								}
						...
						t = Maths.max(Maths.abs(A.get(i, na)), Maths.abs(A.get(i, nn)));
						if (Maths.EPSILON * t * t > 1) {
							for (j = i; j <= nn; j++) {
								A.div(j, na, t);
								A.div(j, nn, t);
