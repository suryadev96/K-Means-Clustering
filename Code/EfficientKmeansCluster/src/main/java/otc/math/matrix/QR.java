...
public class QR {
	...
	/**
	 * The diagonal of R for this implementation. For netlib based QR, this is the
	...
	public void solve(double[] b, double[] x) {
		if (b.length != qr.nrows()) {
			throw new IllegalArgumentException(String.format(
					"Row dimensions do not agree: A is %d x %d, but B is %d x 1", qr.nrows(), qr.nrows(), b.length));
		}
	...
	public void solve(DenseMatrix B) {
		if (B.nrows() != qr.nrows()) {
			throw new IllegalArgumentException(
					String.format("Row dimensions do not agree: A is %d x %d, but B is %d x %d", qr.nrows(), qr.nrows(),
							B.nrows(), B.ncols()));
		}
