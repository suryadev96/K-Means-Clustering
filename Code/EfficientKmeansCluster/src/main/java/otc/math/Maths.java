...
import otc.sort.QuickSelect;
import otc.sort.QuickSort;
import otc.sort.SortUtils;
...
public class Maths {
     ...
     */
    private static ThreadLocal<otc.math.Random> random = new ThreadLocal<otc.math.Random>() {
        protected synchronized otc.math.Random initialValue() {
            if (firstRNG) {
                ...
                return new otc.math.Random(seed);
            }
        }
    };
    ...
    public static double logChoose(int n, int k) {
        ...
        return Maths.logFactorial(n) - Maths.logFactorial(k) - Maths.logFactorial(n - k);
    }
    ...
    public static void pdist(double[][] x, double[][] dist, boolean squared, boolean half) {
        ...
        if (n < 100) {
            ...
            List<PdistTask> tasks = new ArrayList<PdistTask>();
    ...
    public static double squaredDistance(SparseArray x, SparseArray y) {
        ...
        while (it1.hasNext()) {
            sum += Maths.sqr(it1.next().x);
        }

        while (it2.hasNext()) {
            sum += Maths.sqr(it2.next().x);
        }
    ...
    public static double[][] cov(double[][] data) {
        return cov(data, Maths.colMeans(data));
    }
    ...
    public static double[][] cor(double[][] data) {
        return cor(data, Maths.colMeans(data));
    }
    ...
    public static void standardize(double[][] x) {
        ...
        for (int j = 0; j < p; j++) {
            for (int i = 0; i < n; i++) {
                scale[j] += Maths.sqr(x[i][j]);
            }
            scale[j] = Maths.sqrt(scale[j] / (n-1));

            if (!Maths.isZero(scale[j])) {
                for (int i = 0; i < n; i++) {
                    x[i][j] /= scale[j];
                }
            }
    ...
    public static double GoodTuring(int[] r, int[] Nr, double[] p) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            Xsquares += Maths.sqr(logR[i] - meanX);
        }
        ...
        for (int j = 0; j < len; ++j) {
            ...
            if (!indiffValsSeen) {
                ...
                if (Math.abs(x - y) <= CONFID_FACTOR * Maths.sqrt(Maths.sqr(r[j] + 1.0) * n / Maths.sqr(Nr[j]) * (1 + n / (double) Nr[j]))) {
                    indiffValsSeen = true;
                } else {
                    p[j] = x;
                }
    ...
    public static boolean equals(float[][] x, float[][] y, float epsilon) {
        ...
        for (int i = 0; i < x.length; i++) {
            for (int j = 0; j < x[i].length; j++) {
                if (Maths.abs(x[i][j] - y[i][j]) > epsilon) {
                    return false;
                }
            }
        }
